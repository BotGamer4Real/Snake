...
import {
  ...
  MOVE_START,
  RESTART_EVENT,
} from "@/game/swarm/events";

const idleHud: SwarmHud = {
  score: 0,
  highScore: 0,
  lives: 3,
  wave: 1,
...
export default function SwarmCanvas({ onBack }: { onBack: () => void }) {
  type GameHandle = {
    events: {
      emit: (event: string, ...args: unknown[]) => void;
      on: (event: string, fn: (payload: SwarmHud) => void) => void;
      off: (event: string, fn: (payload: SwarmHud) => void) => void;
    };
    scale: { refresh: () => void };
  ...
  const [hud, setHud] = useState<SwarmHud>(idleHud);
  const { highScores } = useAuth();
  const best = highScores.swarm;
  ...
  useEffect(() => {
    ...
    if (!parent) return;
    let cancelled = false;
    const onHud = (payload: SwarmHud) => setHud(payload);

    (async () => {
      const { createSwarmGame } = await import("@/game/swarm/createGame");
      if (cancelled || !parentRef.current) return;
      const game = createSwarmGame(parentRef.current);
    ...
    <div className="flex h-dvh max-h-dvh w-full max-w-[760px] flex-col overflow-hidden px-3 pt-[max(0.5rem,env(safe-area-inset-top))] pb-[max(0.5rem,env(safe-area-inset-bottom))] sm:h-auto sm:max-h-none sm:px-4 sm:py-6">
      <div className="flex shrink-0 items-start justify-between gap-2">
        ...
        <div className="flex items-start gap-1.5 sm:gap-2">
          ...
          <Stat label="Wave" value={hud.wave} />
          <button
            type="button"
            onClick={onBack}
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center py-2 sm:py-4">
        ...
        >
          <div className="pointer-events-none absolute -inset-3 rounded-[32px] bg-amber-400/10 blur-2xl sm:-inset-6" />
          <div
            ref={parentRef}
            className="absolute inset-0 overflow-hidden rounded-[20px] border border-amber-200/15 shadow-[0_30px_80px_rgba(0,0,0,0.55),inset_0_1px_0_rgba(255,255,255,0.12)] sm:rounded-[24px]"
          />
          {hud.countdown != null && !dead && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-[20px] bg-slate-950/35 sm:rounded-[24px]">
              <p className="font-mono text-6xl font-semibold text-white drop-shadow-[0_0_24px_rgba(251,191,36,0.55)] sm:text-7xl">
                {hud.countdown}
              </p>
            </div>
          )}
          {dead && (
            ...
            >
              <div className="mx-4 w-full max-w-[16rem] rounded-3xl border border-white/15 bg-slate-950/85 px-5 py-6 text-center shadow-2xl">
                ...
                <button
                  type="button"
                  className="mt-5 h-11 w-full rounded-full bg-gradient-to-r from-amber-300 to-violet-300 text-sm font-semibold tracking-[0.18em] text-slate-950 uppercase"
                  onClick={(event) => {
  ...
  return (
    <div
      className={`min-w-[52px] rounded-2xl border px-2 py-1.5 text-right sm:min-w-[68px] sm:px-3 sm:py-2 ${
        accent ? "border-amber-300/25 bg-amber-400/10" : "border-white/10 bg-white/5"
      }`}
    >
