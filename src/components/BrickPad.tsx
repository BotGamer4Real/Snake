...
import type { BrickDir } from "@/game/brick/events";

export function BrickPad({
  onMoveStart,
  onMoveEnd,
  onLaunch,
}: {
  onMoveStart: (dir: BrickDir) => void;
  onMoveEnd: (dir: BrickDir) => void;
  onLaunch: () => void;
}) {
  return (
    <div className="flex items-end justify-center gap-8 select-none">
      ...
      <button
        type="button"
        aria-label="launch"
        className="mb-1 flex h-[5.2rem] w-[5.2rem] items-center justify-center rounded-full border border-sky-200/25 bg-[linear-gradient(180deg,rgba(56,189,248,0.28),rgba(255,255,255,0.04))] text-sky-100 shadow-[0_10px_24px_rgba(0,0,0,0.35),inset_0_1px_0_rgba(255,255,255,0.25)] touch-none select-none active:translate-y-[1px] active:brightness-75 sm:h-16 sm:w-16"
      ...
      >
        <span className="text-sm font-semibold tracking-[0.14em] uppercase">Go</span>
      </button>
    </div>
  );
}
