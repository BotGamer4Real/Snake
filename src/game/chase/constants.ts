...
export const PLAYER_STEP_MS = 207;
export const HUNTER_STEP_MS = 224;
export const FRIGHT_STEP_MS = 294;
export const EATEN_STEP_MS = 70;
...
export function playerStepMs(level: number): number {
  return Math.max(140, PLAYER_STEP_MS - level * 2);
}

export function hunterStepMs(level: number, frightened: boolean, eaten: boolean): number {
  if (eaten) return EATEN_STEP_MS;
  if (frightened) return Math.max(210, FRIGHT_STEP_MS - level * 2);
  return Math.max(150, HUNTER_STEP_MS - level * 3);
}
